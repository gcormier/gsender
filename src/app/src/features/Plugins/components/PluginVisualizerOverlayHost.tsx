...
const OverlayPanel = ({
	...
	return (
		<div className="absolute right-4 top-4 z-[10001] flex h-[420px] w-80 max-w-[calc(100%-2rem)] flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-[0_10px_30px_rgba(0,_0,_0,_0.35)] dark:border-dark-lighter dark:bg-dark">
...
const PluginVisualizerOverlayHost = ({ baseBottomPx, leftPx }: Props) => {
	...
	// Drop open state for plugins that are no longer available (disabled/removed).
	useEffect(() => {
		setOpenIds((prev) => {
			const availableIds = new Set(visualizerOverlayPlugins.map((p) => p.id));
			let changed = false;
	...
	return (
		<>
			{visualizerOverlayPlugins.map((plugin, index) => {
				...
				const isOpen = openIds.has(plugin.id);
